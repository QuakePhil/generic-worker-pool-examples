...
func ExampleSleepWorker() {
	result := pool.New[int, int](sleepTest, SleepWorker{}).Wait(1)
...
func ExampleSleepWorkers() {
	result := pool.New[int, int](sleepTest, SleepWorker{}).Wait(10)
	fmt.Println("Slept", result, "times")
	// Output: Slept 10 times
}

func ExamplePrimesWorker() {
	result := pool.New[PrimesRange, int](primesTest, PrimesWorker{}).Wait(1)
	fmt.Println("Found", result, "primes")
	// Output: Found 1270607 primes
}

func ExamplePrimesWorkers() {
	result := pool.New[PrimesRange, int](primesTest, PrimesWorker{}).Wait(1000)
	fmt.Println("Found", result, "primes")
	// Output: Found 1270607 primes
}
